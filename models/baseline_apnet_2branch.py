...
class Resnet_Backbone(nn.Module):
    ...
    def forward(self, x):
        ...
        y5 = self.resnet_layer4(y3)
        y5 = self.att_ss5(y5)
        y5 = self.BN_5(y5)
        y5 = self.att_s5(y5)
        y5 = self.BN5(y5)
        y = self.att5(y5)
        y5 = y5 * y.expand_as(y5)

        return x, y5


class baseline_apnet_2branch(nn.Module):
    def __init__(self, num_classes):

        self.num_classes = num_classes

        super(baseline_apnet_2branch, self).__init__()
        ...
        # part
        self.parts = 6
        self.avgpool_part = nn.AdaptiveAvgPool2d((self.parts, 1))
    ...
    def forward(self, x):
        ...
        x, y5 = self.backbone(x)  # (batch_size, 2048, 16, 8)
        ...
        # parts --------------------------------------------------------------------------
        features_G = self.avgpool_part(y5)  # tensor g([N, 2048, 6, 1])
