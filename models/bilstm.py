...
model_urls["inception_v3_google"] = model_urls["inception_v3_google"].replace(
    "https://", "http://"
)
...
class FullBiLSTM(nn.Module):
    ...
    def forward(
        self, images, seq_lens, im_lookup_table, txt_lookup_table, hidden, texts
    ):
        """Do a forward pass.
        ...
        txt_feats_matrix = autograd.Variable(
            torch.zeros(len(images), word_feats.size()[1])
        )
        ...
        for i in range(txt_feats_matrix.size(0)):
            ...
            txt_feats_matrix[
                i,
            ] = torch.mean(word_feats[table_idxs[i]], 0)
        ...
        return (
            packed_feats,
            (im_feats, txt_feats_matrix),
            self.lstm(packed_feats, hidden),
        )
    ...
    def create_packed_seq(self, feats, seq_lens, im_lookup_table):
        ...
        if feats.is_cuda:
            seqs = autograd.Variable(
                torch.zeros((len(seq_lens), max(seq_lens), feats.size()[1]))
            ).cuda()
        else:
            seqs = autograd.Variable(
                torch.zeros((len(seq_lens), max(seq_lens), feats.size()[1]))
        ...
        seqs = seqs[
            sorted(range(len(seq_lens)), key=lambda k: seq_lens[k], reverse=True), :
        ]
        ...
        return pack_padded_sequence(
            seqs, ordered_seq_lens, batch_first=self.batch_first
        )
