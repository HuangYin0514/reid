...
class Engine(object):
    ...
    def __init__(
        self, datamanager, model, optimizer=None, scheduler=None, use_gpu=True
    ):
    ...
    def run(
        ...
        if visactmap:
            self.visactmap(
                testloader,
                save_dir,
                self.datamanager.width,
                self.datamanager.height,
                print_freq,
            )
            return
        ...
        for epoch in range(start_epoch, max_epoch):
            self.train(
                epoch, max_epoch, trainloader, fixbase_epoch, open_layers, print_freq
            )
    ...
    def test(
        ...
        if multi_eval and len(targets) > 1:
            print("##### Evaluating multiple datasets. #####")
            queryloaders = []
            galleryloaders = []
            for name in targets:
                queryloaders.append(testloader[name]["query"])
                galleryloaders.append(testloader[name]["gallery"])

            rank1 = self._evaluate_multi(
                epoch,
                queryloader=queryloaders,
                galleryloader=galleryloaders,
                dist_metric=dist_metric,
                normalize_feature=normalize_feature,
                save_dir=save_dir,
                use_metric_cuhk03=use_metric_cuhk03,
    ...
    def _evaluate_multi(
        ...
        for ql in queryloader:
            for batch_idx, data in enumerate(ql):
        ...
        for gl in galleryloader:
            for batch_idx, data in enumerate(gl):
                imgs, pids, camids = self._parse_data_for_eval(data)
                if self.use_gpu:
                    imgs = imgs.cuda()
                end = time.time()
        ...
        self.writer.add_scalar("Test/mAP", self.map_v.val, epoch)
        self.writer.add_scalar("Test/rank1", self.rank1_v.val, epoch)
